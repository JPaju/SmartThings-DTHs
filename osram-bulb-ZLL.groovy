...
metadata {
	definition (name: "Osram RGBW ZLL", namespace: "JPaju", author: "Jaakko Paju") {
        ...
        attribute "noResponses", "number"
        attribute "switchColor", "string"

        fingerprint profileId: "C05E", inClusters: "0000,0003,0004,0005,0006,0008,0300,1000,FC0F", outClusters: "0019"
	}
...
def parse(String description) {
   log.info "description is $description"
    ...
    sendEvent(name: "noResponses", value: 0)
...
def refresh() {
    ...
    def noResponses = device.currentValue("noResponses")
    if(noResponses == null) { 
    	sendEvent(name: 'noResponses', value: 1)
    }
    else { 
    	sendEvent(name: 'noResponses', value: noResponses + 1)
    }
    if(noResponses > 2) { 
    	sendEvent(name: "switch", value: "off")
        sendEvent(name: "switchColor", value: "off", displayed: false)
...
def setLevel(value, duration = 21) {
	...
	else if (device.currentValue("switch") == "off" && noResponses < 2) {
		sendEvent(name: "switch", value: "on")
